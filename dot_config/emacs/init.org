...
All-the-icons dropped — nerd-icons is its modern, terminal-capable successor and is already wired into completion/dired/ibuffer. Install fonts once with =M-x nerd-icons-install-fonts=. (FiraCode Nerd Font is already the user's default per the Font section.)
...
** emacsclient file placement

Files opened from a shell (=e foo=, =$EDITOR=) arrive through the server. With
=server-window= at its default nil, the server uses =pop-to-buffer=, which on a
tall frame splits the window and drops the file in the bottom half. Reuse the
...
* LSP
Built-in Eglot is the LSP client. =eglot-booster= speeds up JSON-RPC transport (needs =emacs-lsp-booster= binary in $PATH).
...
Jinx offers enchant-based, just-in-time spell checking. Replaces flyspell + ispell.

Setup on a fresh machine: ~brew install enchant pkg-config~, then ~M-x jinx-install~.
...
#+RESULTS:
** AI (claude-code-ide)
Drives the Claude Code CLI over MCP; exposes Emacs tools (xref, tree-sitter, project, diagnostics) to Claude and shows edits via ediff. Uses the existing vterm backend; requires the =claude= CLI on PATH.
...
* Local packages
Emacs front ends I maintain alongside their Neovim originals. Both live on
=feat/emacs-port= branches that are not pushed yet, so straight.el has
nothing to fetch — they load straight from the working tree. The
=file-directory-p= guards mean checking those repos back out to =main=
(where =emacs/= does not exist) degrades to "the package is absent" rather
than breaking startup.

** dataview
Opens =.parquet=, =.csv=, =.gpkg= and friends as duckdb-rendered tables
instead of raw bytes. Needs =duckdb= on =PATH=, plus a one-time
=duckdb -c "INSTALL spatial;"= for the GDAL-backed formats.

=dataview-setup= is what installs the =file-name-handler-alist= entry, so
it has to run — merely loading the library does nothing.
...
** annotate-markdown
Live browser preview for markdown, with in-browser commenting that writes
=> [!NOTE] Comment:= lines back into the buffer. No external runtime — the
server is Emacs's own =make-network-process=.
...
Show the preview inside Emacs rather than in the external browser. The page
is rendered entirely client-side by =markdown-it=, =mermaid= and
=highlight.js=, so it needs a real JavaScript engine -- =eww= would show an
empty shell. =xwidget-webkit= is one, embedded in a buffer.

Scoped deliberately: the advice let-binds =browse-url-browser-function= for
the duration of this one command, so every other link Emacs opens still goes
to the real browser. Set =jmb/annotate-markdown-use-xwidget= to nil to go
back without unwinding anything.
...
* Custom-file
Loaded just before the terminal theme apply so any =custom.el= face settings can't fight with =jmb/apply-theme=.
