...
- What happened: a paper-ingest loop, session 1. Build makers emitted graph payloads referencing ~13 existing node ids as link anchors — most BEYOND the seed list I gave them (they found them via a knowledge-graph query). The per-paper independent verifiers each reported "confirmed anchors exist via a knowledge-graph lookup". If even one id were slightly wrong, the ingest would have auto-minted a dangling stub, silently failing the loop's own C5 (links must target pre-existing nodes) and C8 (no orphans/stubs). The artifact-existence check (does the file/payload exist) would NOT catch this — the payload existed and was well-formed; the problem is whether its *references resolve*.
...
- What happened: TWICE a subagent's done-report claimed artifacts it did not write. (1) an audit loop: a repair agent claimed a fix but edited nothing. (2) a research-notes loop, session 1: the batch-3 maker reported "7 research notes written"; 0 existed on disk. Both were caught only by the independent verifier — each costing a full verifier round (and in case 2 a repair iteration) to discover a missing file.
...
- Fix applied: added an "Artifact-existence check (before the verifier)" sub-step to SESSION step 1 — orchestrator `ls`/`wc -l`s the maker's claimed files before spending a verifier round; missing → re-run the maker first. Also a project General rule in that loop's STATE.md.
...
- What happened: a coverage loop, session 1, a verifier subagent was asked to grade ALL 6 criteria (mechanical: id/endpoint/created_by/dedup/ledger-completeness over 3 payloads + a 119-node snapshot; AND semantic: deep-mined evidence quality of 40 edges across 22 notes). It ran ~29 min / 40 tool-calls and returned a non-verdict partial message (cut off before the verdict block). No way to resume that agent (no SendMessage in this harness).
...
- What happened: a research loop, session-2 verifier (sonnet) correctly returned FAIL on C5, but its evidence line also asserted "no payload edges exist to check (edges arrays are empty)" — both payloads actually carried 11 and 12 explicit `edges[]` entries. The verdict was right; the structural aside was fabricated from a glance. Orchestrator caught it only by running a direct DB/JSON query during repair.
...
- What happened: a dataset-generation loop. During de-risk I (orchestrator) introduced an out-of-band go/no-go metric — a "best-vote-vs-human agreement ≥ threshold" score — that was NOT one of GOAL.md §2's criteria (the real quality bar there was criterion 6, visual plausibility). I then treated the proxy as a hard blocker and spent ~4 iterations chasing it. The proxy was both stricter than the goal (its threshold sat below the human-human agreement floor) AND misleading (it rewarded a method that scored well on the proxy and penalized a method that was actually correct on the real criterion). Reframing verification back to the actual GOAL criterion (visual judgment) instantly flipped the method choice and the dataset passed all its criteria in the same session.
...
## 2026-06-19 · de-risk validated on non-representative samples → false GO, full build fails
- What happened: a dataset-generation loop. Each de-risk validated a gate on 2 hand-picked samples, returned GO, then the full build failed because the de-risk samples lacked the failure morphology. One de-risk used 2 samples of a single dense morphology → missed a failure mode absent from that morphology (the full build surfaced it). Another used 2 samples of a different uniform morphology → missed a distinct failure mode that only appears elsewhere. Fired 3×.
- Why (root cause in the skill's instructions): SESSION lets a cheap de-risk stand in for the full grader, but never requires the de-risk sample to SPAN the morphologies the full criteria will see. A GO on a narrow sample is a false pass.
- Fix applied (workaround): after the 2nd occurrence, started demanding de-risk samples include the specific failure morphology being guarded against — but only reactively.
- Suggested structural fix (needs user approval before editing SKILL.md): in SESSION, when a maker runs a cheap de-risk as a proxy for the hard gate, require it to cover ≥1 instance of each known failure mode (or explicitly log which modes it does NOT cover, so the orchestrator treats GO as partial). Tie to the existing "no silent caps" principle.
- Occurrences: 3
- Related: a post-hoc-gating pipeline (each iteration adds a rule to veto one bad class) tends to a whack-a-mole ceiling — every gate exposes a new residual. When 3 consecutive iterations each fix one substrate and reveal another, escalate to a learned/root-cause fix rather than another gate. (Recommend the verifier surface "new failure class vs same class" so the orchestrator can detect the pattern.)
